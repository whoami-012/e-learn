...
class TeacherDashboardProvider with ChangeNotifier {
  ...
  Future<void> fetchCourses(String? facultyId) async {
    ...
    try {
      ...
      if (facultyId != null) {
        _allCourses = fetchedCourses.where((c) => c.facultyId == facultyId).toList();
      } else {
  ...
  List<DashboardStat> get stats {
    ...
    final revenue = _allCourses.fold<double>(0, (sum, c) => sum + (c.isFree ? 0 : c.price * 10));
  ...
  List<DashboardCategory> get categories {
    return [
      DashboardCategory(title: 'Development', courses: 12, icon: Icons.code, baseColor: const Color(0xFF3B82F6)),
      DashboardCategory(title: 'Cybersecurity', courses: 5, icon: Icons.security, baseColor: const Color(0xFFEF4444)),
      DashboardCategory(title: 'Design', courses: 8, icon: Icons.palette_outlined, baseColor: const Color(0xFFA855F7)),
      DashboardCategory(title: 'Business', courses: 4, icon: Icons.work_outline, baseColor: const Color(0xFFF97316)),
    ];
  }
}
